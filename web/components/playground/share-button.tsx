...
export function ShareButton({
  ...
  const generateShareText = useCallback(() => {
    ...
    text += `🎯 Overall Risk: ${risk?.toUpperCase() || "Unknown"}\n\n`;
    text += `📊 Clause Breakdown:\n`;
    text += `• ${highRisk} High Risk\n`;
    text += `• ${mediumRisk} Medium Risk\n`;
    text += `• ${lowRisk} Low Risk\n\n`;
    
    if (summary) {
      text += `📝 Summary:\n${summary}\n\n`;
    }

    text += `🔍 Analyzed with ContractCoach AI`;
  ...
  const shareToLinkedIn = useCallback(() => {
    const text = `I just analyzed a contract using ContractCoach AI! The analysis found ${clauses.length} key clauses with an overall ${risk} risk level. AI-powered contract review is the future! 🚀`;
